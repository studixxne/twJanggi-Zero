...
class TwJanggiEncoder:
    def __init__(self, T):
        self.state_queue = deque(maxlen=T)
        self.M = 21
        self.L = 2
        self.T = T

    # (4, 3, in_ch)의 Tensor로 변환
    def get_tensor(self, state):
        board, taken_piece, king_enter, turn, repeat, player = state
        self.state_queue.append(state)
        states = []

        # M*T개 쌓기
        for _ in range(self.T-len(self.state_queue)):
            states.append(np.zeros((self.M, 4, 3)))

        for s in self.state_queue:
            s_board, s_taken_piece, s_king_enter, s_turn, s_repeat, _ = self._get_flip_state(s, player)
            layers = []
        ...
        states.append(np.zeros((1, 4, 3)) if player == 1 else np.ones((1, 4, 3)))
        # 총 이동 수
        states.append(np.full((1, 4, 3), turn))
        
        # (1, M*T+L, 4, 3)의 텐서 반환
        x = torch.tensor(np.concatenate(states, axis=0), dtype=torch.float32).unsqueeze(0)
        return x
